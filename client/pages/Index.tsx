...
export default function Index() {
  ...
  const totalBalance = useMemo(
    () => demoAccounts.reduce((s, a) => s + a.balance, 0),
    [],
  );
  ...
  const history = useMemo(() => {
    ...
    return days.map((d) => {
      ...
      const income = demoTransactions
        .filter((t) => new Date(t.date).getMonth() === monthIdx && t.amount > 0)
        .reduce((s, t) => s + t.amount, 0);
      const spend = Math.abs(
        demoTransactions
          .filter(
            (t) => new Date(t.date).getMonth() === monthIdx && t.amount < 0,
          )
          .reduce((s, t) => s + t.amount, 0),
      );
      return {
          ...
          <ul className="space-y-2">
            {demoAccounts.map((a) => (
              <li key={a.id} className="flex items-center justify-between">
                ...
                <span
                  className={`text-sm ${a.balance < 0 ? "text-destructive" : "text-emerald-600 dark:text-emerald-400"}`}
                >
